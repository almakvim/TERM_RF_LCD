...
//=====================================================================================
void shtc3_proc(void)
{
    static uint32_t timeout_proc = 0;
//    static uint16_t flag_start = 0;
    
    if((timeout_proc+5000 <= HAL_GetTick())&&(Setup.humd))
    {
...
//            if (shtc3_perform_measurements_low_power(&hi2c1, &temp, &hum)) 
            {
                dev_var.shtc3_hum = hum;
                dev_var.shtc3_temp = temp;
            }    
//        }
